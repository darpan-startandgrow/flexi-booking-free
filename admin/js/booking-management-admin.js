...
class BMAdminCore {
    ...
    constructor() {
        jQuery(document).ready(async ($) => { await this.init($); });
    }
    ...
    async init($) {
        ...
        $(document).on('click', '#editsvc', function () {
        ...
        $(document).on('click', '#delsvc', function () {
        ...
        $(document).on('click', '.bm-duplicate-service', function () {
        ...
        $(document).on('click', '#primary_email_modal .close', function () {
        	$(document).find('#primary_email_modal').removeClass('active-modal');
        	$(document).find('#is_main_email').prop('checked', true);
        });

        // Save primary email

        $(document).on('click', '.save_primary_email', function () {
        	...
        	$(document).find('div.email_div').children('div.field_validate_errortext').html('');
        	$(document).find('div.email_div').children('div.field_validate_errortext').hide();
        	$(document).find("div.field_successtext").html('');
        	$(document).find("div.field_successtext").hide();
        	$(document).find('#primary_email_modal').removeClass('active-modal');
        	...
        	if (id == 0) {
        		...
        		bmRestRequest('bm_save_primary_email_field_key', data, function (response) {
        			...
        			if (jsondata != '' && jsondata != null) {
        				...
        				if (status == true) {
        					hideSettings();
        					$(document).find("div.field_successtext").html(bm_success_object.save_success);
        					$(document).find("div.field_successtext").show();
        				} else {
        					$(document).find('div.email_div').children('div.field_validate_errortext').html(bm_error_object.server_error);
        					$(document).find('div.email_div').children('div.field_validate_errortext').show();
        				}
        			} else {
        				$(document).find('div.email_div').children('div.field_validate_errortext').html(bm_error_object.server_error);
        				$(document).find('div.email_div').children('div.field_validate_errortext').show();
        ...
        //Field section functions
        	...
        	$('.global_timezone_errortext').hide();
        	$('.fetch_services_by_category_order_field_errortext').hide();
        				...
        				if (id != 0) {
        					...
        					bmRestRequest('bm_remove_field', data, function (response) {
        						if (bmSafeParse(response).status == 'deleted') {
        							...
        							// $(".field_successtext").html(bm_success_object.field_remove_success);
        							// $(".field_successtext").show();
        						} else {
        							showMessage(bm_error_object.server_error, 'error');
        							// $('.field_errortext').html(bm_error_object.server_error);
        							// $('.field_errortext').show();
        						}
        					});
        				}
        ...
        $(document).on('click', '.preview_button', function (e) {
        	...
        	bmRestRequest('bm_fetch_preview_form', data, async function (response) {
        ...
        $(document).on('click', '#show-product-dialog', function (e) {
        	e.preventDefault();

        	var data = { 'order_id': getUrlParameter('id'), 'nonce': bm_ajax_object.nonce };
        	bmRestRequest('bm_fetch_ordered_product_details', data, function (response) {
        ...
        $(document).on("click", "#remove-extra-product", function () {
        		...
        		bmRestRequest('bm_fetch_saved_order_search', data, function (response) {
        			...
        			if (saved_search != null && saved_search != "") {
        				...
        				if (typeof(saved_search.services) != "undefined" && saved_search.services != '') {
        						...
        						$('#service_filter').val(servicesArray);
        						$('#service_filter').multiselect('reload');
        				}

        				if (typeof(saved_search.categories) != "undefined" && saved_search.categories != '') {
        					var categoriesArray = saved_search.categories.split(',');
        					$('#category_filter').val(categoriesArray);
        					$('#category_filter').multiselect('reload');
        				}
        			}
        ...
        $(document).on('click', '.booking_overview_tag', function (e) {
        	e.preventDefault();
        });

        // Fetch order search data by date

        $(document).on('click', '#date_search_button', function (e) {
        	e.preventDefault();
        	bm_search_order_data('save_search');
        });
        ...
        $(document).on('click', '#reset_date_search', function (e) {
            e.preventDefault();
        ...
        $(document).on('click', '.export_order_records', function (e) {
        	e.preventDefault();
        ...
        $(document).on('change', '#exportOption', function () {
        	if (this.value == '3') {
        ...
        $(document).on('click', '#checkinexportButton', function () {
        	var exportOption = jQuery('#exportOption').val();
        	var totalPages = parseInt(jQuery('#total_pages').val());
        ...
        // Show module pop up message
        	...
        	$(document).on('change', '#email_attachment', add_email_attachment);
        ...
        // Prevent right click on payment settings page
        	...
        	$('#customer_form').on('submit', function (e) {
        		...
        		customer_form_validation().then(isValid => {
        			if (isValid) {
        				if (!$('input[name="savecust"]').length) {
        					$(this).append('<input type="hidden" name="savecust" value="1">');
        				}
        				this.submit();
        			}
        		});
        ...
        $(document).on('click', '.bm-view-pdf-sample', function(e) {
            e.preventDefault();
            ...
            bmRestRequest('view_pdf_content', {
    ...
    static changeVariableMaxCap($this) {
    ...
    // Change Time as per Selection
    static checkVariableTime($this, slot_data_id = '') {
    ...
    static disableVariableSlot($this) {
    ...
    static confirm_slot_change($this) {
    	if ((getUrlParameter('id') != '')) {
    ...
    static variable_price_validation_submit() {
    ...
    static variable_price_module_validation_submit() {
    ...
    // Variable Service Price Field Validation and Submission
    static variable_stopsales_validation_submit() {
    ...
    static variable_saleswitch_validation_submit() {
    ...
    static validate_capacity_and_submit() {
    ...
    static validate_slots_and_submit() {
    ...
    static bulk_price_validation_submit() {
    ...
    static bulk_variable_price_module_validation_submit() {
    ...
    static bulk_stopsales_validation_submit() {
    ...
    static bulk_saleswitch_validation_submit() {
    ...
    static validate_bulk_capacity_and_submit() {
    ...
    static save_calendar_service_price(year = '', month = '', day = '', date = '', price = '', default_price = '', done = false) {
    ...
    // Save Service Calendar Price
    static save_calendar_service_price_module(year = '', month = '', day = '', date = '', module = '', done = 0) {
    	return 0;
    }

    // Save Service Calendar Stopsales
    static save_calendar_service_stopsales(year = '', month = '', day = '', date = '', stopsales = '', default_stopsales = '', done = false) {
    ...
    static save_calendar_service_saleswitch(year = '', month = '', day = '', date = '', saleswitch = '', default_saleswitch = '', done = false) {
    ...
    static save_calendar_service_max_cap(year = '', month = '', day = '', date = '', capacity = '', default_max_cap = '', done = false) {
    ...
    static save_calendar_variable_time_slots(year = '', month = '', day = '', date = '', time_slots_data = {}, slot_nextindex = '', done = false) {
    ...
    static edit_calendar_service_price(values = [], type = '') {
    ...
    static set_value_in_price_calendar(day, month, year, value, default_value, type) {
    	var text = '';
    ...
    static edit_calendar_service_stopsales(values = [], type = '') {
    ...
    static edit_calendar_service_saleswitch(values = [], type = '') {
    ...
    static edit_calendar_service_max_cap(values = [], type = '') {
    ...
    static edit_calendar_service_variable_time_slots(values = []) {
    	var data = { 'data': values, 'nonce': bm_ajax_object.nonce };
    ...
    static bm_get_service_price() {
    	...
    	if (serviceId !== '') {
    		bmRestRequest('bm_get_service_prices', {
    ...
    static bm_get_service_stopsales() {
    ...
    static bm_get_service_saleswitch() {
    	if (jQuery('.saleswitch_errortext').is(':visible')) jQuery('.saleswitch_errortext').hide();
    	if (jQuery('.saleswitch_update_successtext').is(':visible')) jQuery('.saleswitch_update_successtext').hide();
    	if (jQuery('.saleswitch_update_errortext').is(':visible')) jQuery('.saleswitch_update_errortext').hide();
    	if (jQuery('.variable_hour_errortext').is(':visible')) jQuery('.variable_hour_errortext').hide();
    ...
    // Variable Service Price Ajax on Page Load
    static bm_get_service_time_slots() {
    	jQuery('.time_slot_calendar_errortext').hide();
    	jQuery('.time_slot_update_successtext').hide();
    	jQuery('.time_slot_update_errortext').hide();
    ...
    // Field Tabs
    static fieldTabs(evt, fieldSection) {
    ...
    static get_fieldkey_and_order(type) {
    	jQuery(".field_errortext").html('');
    	jQuery(".field_errortext").hide();
    ...
    static add_bm_field(type, ordering, field_key, primary_mail_key) {
    	jQuery(".field_successtext").html('');
    	jQuery(".field_errortext").html('');
    	jQuery(".field_successtext").hide();
    	jQuery(".field_errortext").hide();
    ...
    static getMinLength(type, settings = null) {
    ...
    static showFieldSettings(type, field_key, ordering, field_position, primary_mail_key, settings = null) {
    ...
    static check_if_any_primary_email($this) {
    ...
    static saveField(id) {
    	...
    	if (validateFields()) {
    		...
    		var data = { 'post': post, 'nonce': bm_ajax_object.nonce };
    		bmRestRequest('bm_save_field_and_setting', data, function (response) {
    ...
    static bm_get_all_field_labels(ordering = []) {
    	jQuery('.field_successtext').hide();
    	jQuery('.field_errortext').hide();
    	jQuery('.field_successtext').html(' ');
    	jQuery('.field_errortext').html(' ');
    ...
    // Template validation
    static add_template_validation() {
    ...
    static bm_add_admin_email_option() {
    ...
    static bm_test_smtp_connection() {
    	...
    	bmRestRequest('bm_test_smtp', data, function (response) {
    ...
    static bm_insert_field_in_email(a) {
    	tinyMCE.activeEditor.execCommand('mceInsertContent', false, a);
    }

    // Clear content of wp editor
    static bm_clear_content_in_wp_editor_body(a) {
    	tinymce.activeEditor.setContent(a);
    }

    // Global General Settings validation
    static global_general_settings_validation() {
    ...
    static bm_fetch_bookable_services(category_id) {
    ...
    static bm_fetch_service_time_slots_by_service_id(service_id) {
    	jQuery('.order_field_errortext').html('');
    	jQuery('.order_field_errortext').hide();
    	jQuery('.all_order_error_text').html('');
    ...
    static bm_fetch_bookable_no_of_slots_by_slot($this) {
    	jQuery('.order_field_errortext').html('');
    	jQuery('.order_field_errortext').hide();
    	jQuery('.all_order_error_text').html('');
    ...
    static calculatePrice(price, quantity) {
    ...
    // Event handler for calculating price of a product
    static bm_fetch_svc_total_price() {
    	jQuery('.order_field_errortext').html('');
    	jQuery('.order_field_errortext').hide();
    	jQuery('.all_order_error_text').html('');
    ...
    static addExtraService(index, product) {
    ...
    static checkExtraQuantityInputMaxValue($this) {
    	var id = jQuery($this).attr('id');
    	var index = Number(id.split('_')[2]);
    ...
    static calculateExtraPrice(listItem, index = '') {
    ...
    // Function to calculate extra service total price based on quantity and price inputs
    static getExtraServicePrice($this) {
    	var id = jQuery($this).attr('id');
    	var index = Number(id.split('_')[3]);
    ...
    static setExtraTotalInputValue() {
    	var totalPrice = 0.00;
    ...
    static add_process_form_validation() {
    	jQuery('.errortext').html('');
    	jQuery('.errortext').hide();
    	var b = 0;
    	var types = [];
    ...
    // Show mail details
    static bm_show_mail_details($this) {
    	var id = jQuery($this).attr('id');
    	...
    	var data = { 'post': post, 'nonce': bm_ajax_object.nonce };
    	bmRestRequest('bm_show_mail_details', data, function (response) {
    ...
    // Show mail body
    static bm_show_email_body($this) {
    	var id = jQuery($this).attr('id');
    	...
    	var data = { 'post': post, 'nonce': bm_ajax_object.nonce };
    	bmRestRequest('bm_show_email_body', data, function (response) {
    ...
    // Open mail body
    static bm_open_email_body($this, $module_type = '') {
    ...
    static bm_resend_email(type='') {
    	...
    	var data = { 'post': post, 'nonce': bm_ajax_object.nonce };
    	bmRestRequest('bm_resend_email', data, function (response) {
    ...
    static remove_email_attachmment($this) {
    	var key = sessionStorage.getItem("current_resend_mail_key");
    	var email_id = jQuery(document).find('#email_id_' + key).val();
    ...
    static remove_unsent_temporary_email_attachment() {
    	jQuery('#resend_email_attachment').val('');
    ...
    static bm_payment_settings_validation() {
    	jQuery('.errortext').html('');
    	jQuery('.errortext').hide();
    	var error = 0;
    ...
    static async bm_get_state_of_country(country, stateField) {
    	if (country) {
    		const data = {
    			country,
    			nonce: bm_ajax_object.nonce
    		...
    		try {
    			const response = await bmRestRequest('get_states', data);
    			stateField.empty();
    ...
    static bm_show_vocuher_booking_info($this) {
    	const data = {
    	...
    	bmRestRequest('bm_fetch_vocuher_booking_info', data)
    ...
    static bm_show_vocuher_gifter_info($this) {
    	const data = {
    		order_id: jQuery($this).attr('id'),
    		nonce: bm_ajax_object.nonce
    	};
    	...
    	bmRestRequest('bm_fetch_vocuher_gifter_info', data)
    		.done(function (response) {
    			jQuery("#voucher-data").empty();
    ...
    static bm_show_vocuher_recipient_info($this) {
    	const data = {
    	...
    	bmRestRequest('bm_fetch_vocuher_recipient_info', data)
    		.done(function (response) {
    			jQuery("#voucher-data").empty();
...
window.changeVariableMaxCap = BMAdminCore.changeVariableMaxCap;
window.showSlots = BMAdminCore.showSlots;
window.showSlotsTiming = BMAdminCore.showSlotsTiming;
window.addTime = BMAdminCore.addTime;
window.checkTime = BMAdminCore.checkTime;
window.checkVariableTime = BMAdminCore.checkVariableTime;
window.disableSlot = BMAdminCore.disableSlot;
window.disableAgeGroup = BMAdminCore.disableAgeGroup;
window.disableVariableSlot = BMAdminCore.disableVariableSlot;
window.hideToSlot = BMAdminCore.hideToSlot;
window.autoTime = BMAdminCore.autoTime;
window.variableAutoTime = BMAdminCore.variableAutoTime;
window.addPriceInfo = BMAdminCore.addPriceInfo;
window.addStopsalesInfo = BMAdminCore.addStopsalesInfo;
window.addSaleswitchInfo = BMAdminCore.addSaleswitchInfo;
window.addCapacityInfo = BMAdminCore.addCapacityInfo;
window.confirm_slot_change = BMAdminCore.confirm_slot_change;
window.addTimeSlotInfo = BMAdminCore.addTimeSlotInfo;
window.variable_price_validation_submit = BMAdminCore.variable_price_validation_submit;
window.variable_price_module_validation_submit = BMAdminCore.variable_price_module_validation_submit;
window.variable_stopsales_validation_submit = BMAdminCore.variable_stopsales_validation_submit;
window.variable_saleswitch_validation_submit = BMAdminCore.variable_saleswitch_validation_submit;
window.validate_capacity_and_submit = BMAdminCore.validate_capacity_and_submit;
window.validate_slots_and_submit = BMAdminCore.validate_slots_and_submit;
window.validateVariableSlots = BMAdminCore.validateVariableSlots;
window.bulk_price_validation_submit = BMAdminCore.bulk_price_validation_submit;
window.bulk_variable_price_module_validation_submit = BMAdminCore.bulk_variable_price_module_validation_submit;
window.bulk_stopsales_validation_submit = BMAdminCore.bulk_stopsales_validation_submit;
window.bulk_saleswitch_validation_submit = BMAdminCore.bulk_saleswitch_validation_submit;
window.validate_bulk_capacity_and_submit = BMAdminCore.validate_bulk_capacity_and_submit;
window.save_calendar_service_price = BMAdminCore.save_calendar_service_price;
window.save_calendar_service_price_module = BMAdminCore.save_calendar_service_price_module;
window.save_calendar_service_stopsales = BMAdminCore.save_calendar_service_stopsales;
window.save_calendar_service_saleswitch = BMAdminCore.save_calendar_service_saleswitch;
window.save_calendar_service_max_cap = BMAdminCore.save_calendar_service_max_cap;
window.save_calendar_variable_time_slots = BMAdminCore.save_calendar_variable_time_slots;
window.remove_time_slot = BMAdminCore.remove_time_slot;
window.edit_calendar_service_price = BMAdminCore.edit_calendar_service_price;
window.set_value_in_price_calendar = BMAdminCore.set_value_in_price_calendar;
window.edit_calendar_service_stopsales = BMAdminCore.edit_calendar_service_stopsales;
window.edit_calendar_service_saleswitch = BMAdminCore.edit_calendar_service_saleswitch;
window.edit_calendar_service_max_cap = BMAdminCore.edit_calendar_service_max_cap;
window.edit_calendar_service_variable_time_slots = BMAdminCore.edit_calendar_service_variable_time_slots;
window.bm_get_service_price = BMAdminCore.bm_get_service_price;
window.bm_get_service_stopsales = BMAdminCore.bm_get_service_stopsales;
window.bm_get_service_saleswitch = BMAdminCore.bm_get_service_saleswitch;
window.bm_get_service_max_cap = BMAdminCore.bm_get_service_max_cap;
window.bm_get_service_time_slots = BMAdminCore.bm_get_service_time_slots;
window.fieldTabs = BMAdminCore.fieldTabs;
window.get_fieldkey_and_order = BMAdminCore.get_fieldkey_and_order;
window.add_bm_field = BMAdminCore.add_bm_field;
window.changeFieldMaxCap = BMAdminCore.changeFieldMaxCap;
window.getMinLength = BMAdminCore.getMinLength;
window.showFieldSettings = BMAdminCore.showFieldSettings;
window.checkFieldKey = BMAdminCore.checkFieldKey;
window.check_if_any_primary_email = BMAdminCore.check_if_any_primary_email;
window.addMoreOption = BMAdminCore.addMoreOption;
window.saveField = BMAdminCore.saveField;
window.hideSettings = BMAdminCore.hideSettings;
window.validateFields = BMAdminCore.validateFields;
window.getFormData = BMAdminCore.getFormData;
window.bm_get_all_field_labels = BMAdminCore.bm_get_all_field_labels;
window.get_field_Settings = BMAdminCore.get_field_Settings;
window.add_template_validation = BMAdminCore.add_template_validation;
window.bm_add_admin_email_option = BMAdminCore.bm_add_admin_email_option;
window.bm_remove_shop_admin_email = BMAdminCore.bm_remove_shop_admin_email;
window.bm_toggle_tab = BMAdminCore.bm_toggle_tab;
window.bm_test_smtp_connection = BMAdminCore.bm_test_smtp_connection;
window.bm_insert_field_in_email = BMAdminCore.bm_insert_field_in_email;
window.bm_clear_content_in_wp_editor_body = BMAdminCore.bm_clear_content_in_wp_editor_body;
window.global_general_settings_validation = BMAdminCore.global_general_settings_validation;
window.bm_fetch_timezone = BMAdminCore.bm_fetch_timezone;
window.setIntlInput = BMAdminCore.setIntlInput;
window.saveProductChanges = BMAdminCore.saveProductChanges;
window.bm_fetch_bookable_services = BMAdminCore.bm_fetch_bookable_services;
window.bm_fetch_service_time_slots_by_service_id = BMAdminCore.bm_fetch_service_time_slots_by_service_id;
window.bm_fetch_bookable_no_of_slots_by_slot = BMAdminCore.bm_fetch_bookable_no_of_slots_by_slot;
window.calculatePrice = BMAdminCore.calculatePrice;
window.bm_fetch_svc_total_price = BMAdminCore.bm_fetch_svc_total_price;
window.bm_fetch_service_extra = BMAdminCore.bm_fetch_service_extra;
window.addExtraService = BMAdminCore.addExtraService;
window.checkExtraQuantityInputMaxValue = BMAdminCore.checkExtraQuantityInputMaxValue;
window.calculateExtraPrice = BMAdminCore.calculateExtraPrice;
window.getExtraServicePrice = BMAdminCore.getExtraServicePrice;
window.setExtraTotalInputValue = BMAdminCore.setExtraTotalInputValue;
window.resetNoOfServiceSelection = BMAdminCore.resetNoOfServiceSelection;
...
window.bm_show_email_body = BMAdminCore.bm_show_email_body;
window.bm_open_email_body = BMAdminCore.bm_open_email_body;
window.bm_resend_email = BMAdminCore.bm_resend_email;
window.add_email_attachment = BMAdminCore.add_email_attachment;
window.remove_email_attachmment = BMAdminCore.remove_email_attachmment;
window.remove_unsent_temporary_email_attachment = BMAdminCore.remove_unsent_temporary_email_attachment;
window.bm_payment_settings_validation = BMAdminCore.bm_payment_settings_validation;
window.change_flexi_language = BMAdminCore.change_flexi_language;
window.bm_get_state_of_country = BMAdminCore.bm_get_state_of_country;
